...
public class Picture {
     ...
     * @throws IOException
     */
    Picture(InputStream input, ImageType sourceType) throws IOException {
     ...
     * @param img
     */
    private Picture(BufferedImage img, ImageType sourceType) {
        this.img = img;
        this.sourceType = (sourceType == null ? ImageType.UNKNOWN : sourceType);
    }
    ...
    /**
     * Generate a new Picture object cropped to a new size
     ...
     * @return Picture cropped to new dimensions
     */
    public Picture crop(int x1, int y1, int x2, int y2) {
        ...
        return new Picture(cropped, sourceType);
    }
     ...
     * @return Picture cropped and resized to a square
     */
    public Picture getResizedToSquare(int width, double cropEdgesPct) {
        ...
        Picture cropped = crop(x1, y1, x2, y2);

        // now resize. we do crop first then resize to preserve detail
        Picture resized = cropped.getResizedToWidth(width);
        cropped.dispose();
     ...
     * @return New Picture object post-softening, unless softenFactor == 0, in which
     * case the same object is returned
     * @throws NotImplementedException Not implemented in this class
     */
    public Picture soften(float softenFactor) {
        if (softenFactor == 0f)
            return this;
        else {
            ...
            return new Picture(cOp.filter(img, null), sourceType);
        }
    }
